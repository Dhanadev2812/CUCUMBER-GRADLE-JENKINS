  ...
  agent {
	...
	post {
		success {  
			...
			body: "<b>Build Status Report</b> <br>Job name : Job ${JOB_NAME} <br>Build No : build ${BUILD_NUMBER} <br>Branch Name :${BRANCH_NAME} <br>Build Result : ${currentBuild.currentResult} <br> Console output : ${BUILD_URL} <br> Log File : ${BUILD_LOG, maxLines=9999, escapeHtml=false}",
        		mimeType: 'text/html',
